...
export function readAgentModelConfigFromEnv(): AgentModelConfig | undefined {
  const raw = readEnv("FOUNDERSOS_AGENT_PROVIDER");
  ...
  const explicitModel = readEnv("FOUNDERSOS_AGENT_MODEL");
  const model =
    explicitModel ??
    (provider === "anthropic" ? "claude-sonnet-4-6" : "gpt-4.1");

  const maxTokens = readEnvInt("FOUNDERSOS_AGENT_MAX_TOKENS") ?? 4096;
...
export function readPrincipalFromEnv(): "interactive" | "autonomous" {
  const raw = readEnv("FOUNDERSOS_PRINCIPAL");
...
export function buildContext(): ToolContext {
  ...
  cached = {
    ...
    actor: autonomous
      ? { kind: "autonomous", runId: readEnv("FOUNDERSOS_RUN_ID") ?? randomUUID() }
